...
- **ci:** Renamed the matrix display names in `.github/workflows/release.yml` (`Build ${{ matrix.target }}` → `Build ${{ matrix.display_name }}`, e.g. `Build Linux x86_64 (glibc)`, `Build macOS aarch64 (Apple Silicon)`, `Build Windows x86_64 (MSVC)`) and `.github/workflows/ci.yml` test job (`Test (${{ matrix.os }})` → `Test (${{ matrix.label }})`, e.g. `Test (Linux)`). The Rust target triple stays unchanged (`x86_64-unknown-linux-gnu` etc.) — asset filenames, R2 `latest/checksums.sha256` entries, and `install.sh`/`install.ps1` all key off it, and the `unknown` segment is the Rust convention for "vendor-agnostic" (glibc binaries run on Debian, Fedora, Arch, Ubuntu, …), not a build-status indicator. This is a UI-only rename so the Actions run list reads cleanly without implying a failure mode.

## [0.4.0](https://github.com/lucasram20/artui/compare/v0.3.10...v0.4.0) (2026-05-31)

Patch release that fixes Windows `latest`-resolution in the install script. The pre-release-suffix branch in the version regex was greedy enough to chew into the architecture portion of the archive filename, so `irm … | iex` resolved to a bogus version on Windows and downloads 404'd against both R2 and the GitHub fallback.

### Fixed

- **install:** `scripts/install.ps1` captured the wrong version from the R2 `latest/checksums.sha256` file because the optional pre-release-suffix branch in the regex (`(?:[-+][0-9A-Za-z.\-]+)?`) was permissive enough to extend across the version-arch boundary in archive names like `artui-0.3.10-aarch64-apple-darwin.tar.gz` — it captured `0.3.10-aarch64-apple` instead of `0.3.10`. The script then built an asset URL that didn't exist on R2 (`artui-0.3.10-aarch64-apple-x86_64-pc-windows-msvc.zip`), missed the mirror, and the GitHub fallback 404'd against the private repo. Anchored the regex to a known architecture token (`aarch64|x86_64|i686|arm64`) so the suffix branch can't cross the boundary. `install.sh`'s `sed` pattern (`[0-9.]*`) was always correct — bug was Windows-only. Workaround on older copies of the script: `irm $R2Base/install.ps1 | iex -ArgumentList -Version v0.3.10`.

### Notes

- Bumped 0.3.10 → 0.4.0 across `Cargo.toml`, `Cargo.lock`, `npm/src/package.json`, `npm/src/package-lock.json`, `npm/package-lock.json`, and the install-script header comments. No other code changes — the freemodel relay, UA fix, and Cloudflare Worker from 0.3.9 / 0.3.10 carry forward unchanged.
